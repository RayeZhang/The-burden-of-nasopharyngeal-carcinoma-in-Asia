...
################### Nordpred prediction
setwd('D:/BAPC/route') 
install.packages("D:/BAPC/BAPC_packages/haraldwf-nordpred-ef83b2c.tar.gz", repos = NULL, type = "source")
install.packages('foreach')
install.packages("D:/BAPC/BAPC_packages/INLA_21.02.23.zip", repos = NULL, type = "source")
#安装BAPC前需要预装一些包
...
rownames(GBD_Asia_female_g) <- rownames(EC_female_incidence_n)
rownames(GBD_Asia_male_g) <- rownames(EC_male_incidence_n)
...
#### --------------------------------------------------------nordpred prediction
male_res <- nordpred(EC_male_incidence_g, GBD_Asia_male_g, 
                     noperiods = 5:6, startestage = 5, startuseage = 5, 
                     cuttrend = c(0, .25, .5, .75,0.75), linkfunc = "power5", recent = NULL)

female_res <- nordpred(EC_female_incidence_g, GBD_Asia_female_g, 
                       noperiods = 5:6, startestage = 5, startuseage = 5,
                       cuttrend = c(0, .25, .5, .75,0.75), linkfunc = "power5", recent = NULL)
...
plot <- ggplot(combined_data, aes(x = year, y = ASR, color = gender, group = gender)) +
  ...
  theme(
    ...
    title = element_text(size = 12, face = "bold", family = "Arial", color = "black"),
    axis.title.x = element_text(size = 12, face = "bold", family = "Arial", color = "black"),
    axis.title.y = element_text(size = 12, face = "bold", family = "Arial", color = "black"),
  ...
  scale_y_continuous(limits = c(1, 5.5), breaks = seq(1, 5.5, by = 1.5))
plot
...
rate_2019 <- EC_male_incidence_n[,30]
...
plot_male <- ggplot() + 
  ...
  labs(x = "Year", y = "Numbers of incidence cases") # 设置轴标题
...
#绘图：
plot(male_res, incidence = F, standpop = NULL, new = TRUE)
ggplot() + 
  geom_line(data = Asia_male_3, aes(x = as.numeric(year), y = number), linetype = 'dashed') +
  geom_ribbon(data = Asia_male_4_5, aes(x = as.numeric(year), ymin = number, ymax = number_up), alpha = 0.4) +
  geom_point(data = EC_male, aes(x = year, y = val))  + 
...
rate_2019 <- EC_female_incidence_n[,30]
rate_2019 <- data.frame(rate_2019)
colnames(rate_2019)[1] <- "number"
rownames(rate_2019) <- c("0 to 4", "5 to 9", "10 to 14", "15 to 19", "20 to 24", "25 to 29",
                         "30 to 34", "35 to 39", "40 to 44", "45 to 49", "50 to 54", "55 to 59",
...
library(showtext)  # 用于添加字体

library(ggplot2)
library(showtext)

# 为了能够在图形中使用系统字体，初始化showtext
showtext_auto(enable = TRUE)
library(ggplot2)
library(showtext)
...
plot_male <- ggplot() + 
  ...
  scale_y_continuous(breaks = seq(10000, 70000, by = 10000), limits = c(10000, 70000)) +
  labs(x = "Year", y = "Numbers of incidence cases") # 设置轴标题
...
#效果相当于：
plot(female_res, incidence = F, standpop = NULL, new = TRUE)
ggplot() + 
  geom_line(data = Asia_female_3, aes(x = as.numeric(year), y = number), linetype = 'dashed') +
  geom_ribbon(data = Asia_female_4_5, aes(x = as.numeric(year), ymin = number, ymax = number_up), alpha = 0.4) +
  geom_point(data = EC_female, aes(x = year, y = val))  + 
...
ggplot() + 
  ...
  geom_ribbon(data = Asia_both_4_5, aes(x = as.numeric(year), ymin = number, ymax = number_up), alpha = 0.4) +
  geom_point(data = EC_both, aes(x = year, y = val))  + 
  geom_line(data=Asia_both_pre_3,aes(x = new_year, y = number),color="green")+ geom_point()
