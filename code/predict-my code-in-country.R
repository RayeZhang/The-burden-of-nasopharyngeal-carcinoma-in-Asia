...
# 修改第19行的名字为'85+'
rownames(EC_male_incidence_n)[19] <- "85+"
# 修改第1行的名字为'0-4'
rownames(EC_male_incidence_n)[1] <- "0 to 4"
#sum
EC_male_incidence_n[19,] <- EC_male_incidence_n[19,] + EC_male_incidence_n[20,] + EC_male_incidence_n[21,]
EC_male_incidence_n <- EC_male_incidence_n[-c(20,21),]
EC_male_incidence_n[1,] <- EC_male_incidence_n[1,] + EC_male_incidence_n[2,] 
EC_male_incidence_n <- EC_male_incidence_n[-c(2),]
# 按年龄从小到大排序
start_ages <- as.numeric(regmatches(rownames(EC_male_incidence_n), regexpr("^\\d+", rownames(EC_male_incidence_n))))# 如果有'85+'这一行，则把它的起始年龄设置为85
if("85+" %in% rownames(EC_male_incidence_n)) {
  start_ages[rownames(EC_male_incidence_n) == "85+"] <- 85
}
EC_male_incidence_n <- EC_male_incidence_n[order(start_ages),]
# adjust disease burden data further
rownames(EC_male_incidence_n) <- age_3
...
# 修改第19行的名字为'85+'
rownames(EC_female_incidence_n)[19] <- "85+"
# 修改第1行的名字为'0-4'
rownames(EC_female_incidence_n)[1] <- "0 to 4"
#sum
EC_female_incidence_n[19,] <- EC_female_incidence_n[19,] + EC_female_incidence_n[20,] + EC_female_incidence_n[21,]
EC_female_incidence_n <- EC_female_incidence_n[-c(20,21),]
EC_female_incidence_n[1,] <- EC_female_incidence_n[1,] + EC_female_incidence_n[2,] 
EC_female_incidence_n <- EC_female_incidence_n[-c(2),]
# 按年龄从小到大排序
start_ages <- as.numeric(regmatches(rownames(EC_female_incidence_n), regexpr("^\\d+", rownames(EC_female_incidence_n))))# 如果有'85+'这一行，则把它的起始年龄设置为85
if("85+" %in% rownames(EC_female_incidence_n)) {
  start_ages[rownames(EC_female_incidence_n) == "85+"] <- 85
}
EC_female_incidence_n <- EC_female_incidence_n[order(start_ages),]
# adjust disease burden data further
rownames(EC_female_incidence_n) <- age_3
...
# organize furthe
GBD_Kuwait_male <- subset(GBD,location_name=="Kuwait" & sex_name == 'Male')
GBD_Kuwait_female <- subset(GBD,location_name=="Kuwait" & sex_name == 'Female')

GBD_Kuwait_male_n <- dcast(data = GBD_Kuwait_male, age_group_name ~ year_id,value.var = c("val"))
GBD_Kuwait_female_n <- dcast(data = GBD_Kuwait_female, age_group_name ~ year_id,value.var = c("val"))

GBD_Kuwait_male_n <- GBD_Kuwait_male_n[,-1]
GBD_Kuwait_female_n <- GBD_Kuwait_female_n[,-1]
#5 years gap
...
GBD_Kuwait_male_g <- function_sum_year5(GBD_Kuwait_male_n,1990,2039,2019)
GBD_Kuwait_female_g <- function_sum_year5(GBD_Kuwait_female_n,1990,2039,2019)
rownames(EC_male_incidence_g) <- rownames(EC_male_incidence_n)
rownames(EC_female_incidence_g) <- rownames(EC_female_incidence_n)
rownames(GBD_Kuwait_female_g) <- rownames(EC_female_incidence_n)
rownames(GBD_Kuwait_male_g) <- rownames(EC_male_incidence_n)

GBD_Kuwait_male_g <- apply(GBD_Kuwait_male_g, c(1,2), as.numeric) %>% as.data.frame()
GBD_Kuwait_female_g <- apply(GBD_Kuwait_female_g, c(1,2), as.numeric) %>% as.data.frame()
...
#### --------------------------------------------------------nordpred prediction
male_res <- nordpred(EC_male_incidence_g, GBD_Kuwait_male_g, 
                     noperiods = 4:6, startestage = 5, startuseage = 5, 
                     cuttrend = c(0, .25, .5, .75), linkfunc = "power5", recent = NULL)

female_res <- nordpred(EC_female_incidence_g, GBD_Kuwait_female_g, 
                       noperiods = 4:6, startestage = 5, startuseage = 5,
                       cuttrend = c(0, .25, .5, .75), linkfunc = "power5", recent = NULL)

##calculate the number of cases and incidene of different ages（男女结预测结果）
...
##预测男女总和（全性别结预测结果）
GBD_Kuwait_all_g<- GBD_Kuwait_female_g+GBD_Kuwait_male_g
All_age_cout<- female_age_count+male_age_count
All_age_rate<- All_age_cout/GBD_Kuwait_all_g*10^5
All_age_standardized<-matrix(nrow=0,ncol=2)%>%as.data.frame()
names(All_age_standardized)<-c("ASR","year")

for(i in 1:ncol (All_age_cout)){
  asr=ageadjust.direct (count= All_age_cout[,i],pop=GBD_Kuwait_all_g[,i],
                        stdpop=wstand)
...
# 假设 combined_data 已经按照您之前的代码创建好，并且 `year` 是字符类型或因子类型。

plot <- ggplot(combined_data, aes(x = year, y = ASR, color = gender, group = gender)) +
  geom_line(data = subset(combined_data, year <= "2015-2019"), aes(linetype = "Solid"), size = 1.5) +
  geom_line(data = subset(combined_data, year > "2010-2014"), aes(linetype = "Dashed"), size = 1.5) +
  geom_point(size = 4) +
  labs(x = "Year", y = "ASMR") +
  theme_bw() +
  theme(
